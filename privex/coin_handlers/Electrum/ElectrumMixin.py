...
class ElectrumMixin(BaseHandler):
    """
    ElectrumMixin - shared code used by both :class:`Electrum.ElectrumLoader` and :class:`Electrum.ElectrumManager`
    ...
    **Copyright**::
    ...
    _bc_defaults = dict(
        host='127.0.0.1', port=7777, user=None, password=None,
    ...
    def _prep_settings(self, reset: bool = False) -> Dict[str, dict]:
        ...
        # log.debug('Loading Electrumd handler settings from Coin objects')
        ...
        # log.debug('Loading Electrumd handler settings from settings.COIND_RPC (if it exists)')
